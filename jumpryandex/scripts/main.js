...

{
	...
	const DOM_COMPONENT_ID = "chadori-mobile-ironsource";

	const HANDLER_CLASS = class MobileIronSource_Handler extends self.DOMElementHandler
	{
		constructor(iRuntime)
		{
			...
			this.AddRuntimeMessageHandlers([
				...
				["validate-integration", e => this._ValidateIntegration(...e)],

				["set-mode", e => this._SetMode(...e)],
				["initialize", e => this._Initialize(...e)],
				["check-placement-if-capped", e => this._CheckPlacementIfCapped(...e)],

				["set-consent", e => this._SetConsent(...e)],
				["set-compliance", e => this._SetCompliance(...e)],
				["set-child-directed", e => this._SetChildDirected(...e)],
				["set-device-id-opt-out", e => this._SetDeviceIdOptOut(...e)],
				
				["set-segment", e => this._SetSegment(...e)],

				["set-video-server-params", e => this._SetVideoServerParams(...e)],
				["clear-video-server-params", e => this._ClearVideoServerParams(...e)],
				["set-should-track-network-state", e => this._SetShouldTrackNetworkState(...e)],

				["set-offerwall-server-params", e => this._SetOfferwallServerParams(...e)],
				["set-offerwall-client-side-callbacks", e => this._SetOfferwallClientSideCallbacks(...e)],

				["show-rewarded-video", e => this._ShowRewardedVideo(...e)],
				["get-video-placement-info", e => this._GetVideoPlacementInfo(...e)],
				["set-dynamic-user-id", e => this._SetDynamicUserId(...e)],
				["check-video-if-available", e => this._CheckVideoIfAvailable(...e)],

				["create-interstitial", e => this._CreateInterstitial(...e)],
				["show-interstitial", e => this._ShowInterstitial(...e)],
				["check-interstitial-if-ready", e => this._CheckInterstitialIfReady(...e)],

				["create-banner", e => this._CreateBanner(...e)],
				["show-banner", e => this._ShowBanner(...e)],
				["hide-banner", e => this._HideBanner(...e)],
				["destroy-banner", e => this._DestroyBanner(...e)],

				["show-offerwall", e => this._ShowOfferwall(...e)],
				["get-offerwall-credits", e => this._GetOfferwallCredits(...e)],

				// [Muted] These are currently not called from the runtime side,
				//		   but available just in case.
				["set-banner-overlap", e => this._SetBannerOverlap(...e)],
				["get-mode", e => this._GetMode(...e)],

				["set-meta-data", e => this._SetMetaData(...e)],
				["set-meta-data-bool", e => this._SetMetaDataBool(...e)]
			]);
		...
		{
			if (typeof window["IronSource"] === "undefined") { return false; }
			return true;
		}

		//** Basic Logging **//

		// [Debug] Log in the console with the Construct 3 plugin name tag.
		_Log(str) { if (!this._isDebug) { return; } console.log("[Mobile IronSource]", str) }

		// [Debug] Log in the console the newly called action.
		_LogAction(name) { this._Log(`Called method "${name}".`); }

		// [Debug] Log in the console the newly called background method.
		_LogBackgroundAction(name) { this._Log(`Called background method "${name}".`); }

		//** Colored Logging **//

		// [Debug] Log in the console the newly received callback response from an action
		//		   or from a background action method.
		_LogCallback(name, callback = {}, isSuccess = true)
		{
            if (!this._isDebug) { return; }
            
            if (isSuccess)
            {
				const title = `[Mobile IronSource] Successful callback for ${name}.`;
                console.log("%c%s", "color: green; background: black", title, callback);
            } 
			else
            {
                const title = `[Mobile IronSource] Failure callback for ${name}.`;
                console.log("%c%s", "color: red; background: black", title, callback);
            }
		}

		// [Debug] Log in the console any information. Sample use case is for event responses.
		_LogInfo(info, data = {})
		{
			if (!this._isDebug) { return; }

			const title = `[Mobile IronSource] ${info}.`;
			console.log("%c%s", "color: white; background: black;", title, data);
		}
		...
		SetEventListeners()
		{
			window.addEventListener("initializationCompleted",            (...e)=>{ this.FireEvent("initializationCompleted", e); });
			
            window.addEventListener("rewardedVideoOpened",                (...e)=>{ this.FireEvent("rewardedVideoOpened", e); });
            window.addEventListener("rewardedVideoClosed",                (...e)=>{ this.FireEvent("rewardedVideoClosed", e); });
            window.addEventListener("rewardedVideoAvailabilityChanged",   (...e)=>{ this.FireEvent("rewardedVideoAvailabilityChanged", e); });
            window.addEventListener("rewardedVideoRewardReceived",        (...e)=>{ this.FireEvent("rewardedVideoRewardReceived", e); });
            window.addEventListener("rewardedVideoFailed",                (...e)=>{ this.FireEvent("rewardedVideoFailed", e); });
            window.addEventListener("rewardedVideoClicked",               (...e)=>{ this.FireEvent("rewardedVideoClicked", e); });
            window.addEventListener("rewardedVideoStarted",               (...e)=>{ this.FireEvent("rewardedVideoStarted", e); });
            window.addEventListener("rewardedVideoEnded",                 (...e)=>{ this.FireEvent("rewardedVideoEnded", e); });

            window.addEventListener("interstitialReady",                  (...e)=>{ this.FireEvent("interstitialReady", e); });
            window.addEventListener("interstitialLoadFailed",             (...e)=>{ this.FireEvent("interstitialLoadFailed", e); });
            window.addEventListener("interstitialOpened",                 (...e)=>{ this.FireEvent("interstitialOpened", e); });
            window.addEventListener("interstitialClosed",                 (...e)=>{ this.FireEvent("interstitialClosed", e); });
            window.addEventListener("interstitialShowSucceeded",          (...e)=>{ this.FireEvent("interstitialShowSucceeded", e); });
            window.addEventListener("interstitialShowFailed",             (...e)=>{ this.FireEvent("interstitialShowFailed", e); });
            window.addEventListener("interstitialClicked",                (...e)=>{ this.FireEvent("interstitialClicked", e); });

            window.addEventListener("bannerLoaded",                       (...e)=>{ this.FireEvent("bannerLoaded", e); });
            window.addEventListener("bannerLoadFailed",                   (...e)=>{ this.FireEvent("bannerLoadFailed", e); });
            window.addEventListener("bannerClicked",                      (...e)=>{ this.FireEvent("bannerClicked", e); });
            window.addEventListener("bannerScreenPresented",              (...e)=>{ this.FireEvent("bannerScreenPresented", e); });
            window.addEventListener("bannerScreenDismissed",              (...e)=>{ this.FireEvent("bannerScreenDismissed", e); });
            window.addEventListener("bannerLeftApplication",              (...e)=>{ this.FireEvent("bannerLeftApplication", e); });

            window.addEventListener("offerwallAvailabilityChanged",       (...e)=>{ this.FireEvent("offerwallAvailabilityChanged", e); });
            window.addEventListener("offerwallOpened",                    (...e)=>{ this.FireEvent("offerwallOpened", e); });
            window.addEventListener("offerwallShowFailed",                (...e)=>{ this.FireEvent("offerwallShowFailed", e); });
            window.addEventListener("offerwallCredited",                  (...e)=>{ this.FireEvent("offerwallCredited", e); });
            window.addEventListener("offerwallCreditsFailed",             (...e)=>{ this.FireEvent("offerwallCreditsFailed", e); });
            window.addEventListener("offerwallClosed",                    (...e)=>{ this.FireEvent("offerwallClosed", e); });

			window.addEventListener("segmentReceived",            		  (...e)=>{ this.FireEvent("segmentReceived", e); });
		}

		// Fire event that has a runtime event trigger condition.
		FireEvent(event, argsList)
		{
			const clonedArgsList = JSON.parse(JSON.stringify(argsList));

			this.PostToRuntime("on-event", [event, clonedArgsList]);
			this._LogInfo(event, argsList);
		}

		// Fire callback that has a runtime event trigger condition.
		FireCallback(event, argsList)
		{
			this.PostToRuntime("on-callback", [event, argsList]);
			this._LogCallback(event, argsList);
		}

		// Fire failure callback.
		// [Note] There is no need to post this to the runtime because
		//		  there is no event trigger for this. If this runs,
		//		  then that means the native code didn't work and 
		//		  is a symptom of a Cordova plugin bug.
		FireFailureCallback(event, argsList)
		{
			this.PostToRuntime("on-failure-callback", [event, argsList]);

			const isSuccess = false; // Failure callback indicator.
			this._LogCallback(event, argsList, isSuccess);
		}

		// Silent callback, however this is only for debugging purposes.
		// [Note] Not currently used at the moment, just reserved.
		FireSilentCallback(argsList)
		{
			
		}

		// Silent failure callback, however this is only for debugging purposes.
		// [Note] If this runs, then that means the native code didn't work and
		//		  is a symptom of a Cordova plugin bug.
		FireSilentFailureCallback(argsList)
		{
			const isSuccess = false; // Failure callback indicator.
			const event = "";
			this._LogCallback(event, argsList, isSuccess);
		...
		_StartSDK(isDebug, appKey, initialization, isBannerEnabled, isInterstitialEnabled, isVideoEnabled, isOfferwallEnabled,
						   isShouldTrackNetworkState, isBannerOverlap, isToReceiveOfferwallCallbacks)
		{
			if (!this._isMobile) { this._Log("You need to export to mobile in order to run."); return; }

			// Cache the IronSource object.
			this._IronSource = window["IronSource"];
			// Set instance variables.
			this._isDebug = isDebug;

			// Interpret properties
			const isAutoInitialize = initialization === 0; // ["automatic", "manual"]

			// Start mutable tasks, meaning can be changed later through actions.
			this._SetMode(isVideoEnabled, isInterstitialEnabled, isOfferwallEnabled, isBannerEnabled);
			this._SetOfferwallClientSideCallbacks(isToReceiveOfferwallCallbacks);

			// Start immutable tasks, meaning CANNOT be changed later.
			this._SetShouldTrackNetworkState(isShouldTrackNetworkState);
			this._SetBannerOverlap(isBannerOverlap);

			// Callback to main thread that the plugin is ready.
			this.PostToRuntime("on-mobile-sdk", [isVideoEnabled, isInterstitialEnabled, isOfferwallEnabled, isBannerEnabled]);

			// Start after tasks.
			if (isAutoInitialize) 
			{ 
				const userIdType = 0; // [Advertising, IronSource, Specify]
				const userId = ""; // Use user id if manual. [Note] Empty string is null.
				this._Initialize(appKey, userIdType, userId); 
			}
		}
		...
		{
            this._IronSource["validateIntegration"]((e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Validate Integration");
		}

		//** Settings **//

		_SetMode(isVideoEnabled, isInterstitialEnabled, isOfferwallEnabled, isBannerEnabled)
		{
            const data = {
                "reward": isVideoEnabled,
                "interstitial": isInterstitialEnabled,
                "offerwall": isOfferwallEnabled,
                "banner": isBannerEnabled
            }

            this._IronSource["setMode"](data, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogBackgroundAction("Set Mode");
		}

		// [Muted] This is not an action method, this is for background tasks.
		_GetMode()
		{
			const EVENT_NAME = "get-mode";

            this._IronSource["getMode"]((...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogBackgroundAction("Get Mode");
		}

		// [Muted] This is not an action method, this is for background tasks.
		_SetBannerOverlap(isToOverlap)
		{
            this._IronSource["setBannerOverlap"](isToOverlap, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogBackgroundAction("Set Banner Overlap");
		}

		//** General **//

		_Initialize(appKey, userIdType, userId)
		{
			const EVENT_NAME = "initialize";
			
			// @userIdType = [Advertising, IronSource, Specify]

			// [Note] Included in the success callback is the '_GetMode()' method, this is to make a redundancy sync of the enabled advertisement units. However, this is not necessary, this is only to make sure.
            this._IronSource["initialize"](appKey, userIdType, userId, (...e) => { this.FireCallback(EVENT_NAME, e); this._GetMode(); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Initialize");
		}

		_CheckPlacementIfCapped(advertisementType, placementName)
		{
			const EVENT_NAME = "check-placement-if-capped";

			switch (advertisementType)
			{
				case 0: // Rewarded Video
					this._IronSource["isVideoPlacementCapped"](placementName, (...e) => { this.FireCallback(`${EVENT_NAME}-rewarded-video`, e); }, (...err) => { this.FireFailureCallback(`${EVENT_NAME}-rewarded-video`, err) });
					this._LogAction("Check Placement If Capped (Rewarded Video");
					break;
				case 1: // Interstitial
					this._IronSource["isInterstitialPlacementCapped"](placementName, (...e) => { this.FireCallback(`${EVENT_NAME}-interstitial`, e); }, (...err) => { this.FireFailureCallback(`${EVENT_NAME}-interstitial`, err) });
					this._LogAction("Check Placement If Capped (Interstitial");
					break;
				case 2: // Banner
					this._IronSource["isBannerPlacementCapped"](placementName, (...e) => { this.FireCallback(`${EVENT_NAME}-banner`, e); }, (...err) => { this.FireFailureCallback(`${EVENT_NAME}-banner`, err) });
					this._LogAction("Check Placement If Capped (Banner");
					break;
			}
		}
		...
		_SetConsent(isGiven)
		{
            this._IronSource["setConsent"](isGiven, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Consent");
		}

		_SetCompliance(isDoNotSell)
		{
            this._IronSource["setCompliance"](isDoNotSell, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Compliance");
		}

		_SetChildDirected(isChildDirected)
		{
            this._IronSource["setChildDirected"](isChildDirected, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Child Directed");
		}

		_SetDeviceIdOptOut(isOptOut)
		{
            this._IronSource["setDeviceIdOptOut"](isOptOut, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Device Id Opt-Out");
		}		

		_SetSegment(segmentObject)
		{
            this._IronSource["setSegment"](segmentObject, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Segment");
		}

		//** Advanced Settings - Rewarded Video **//

		_SetVideoServerParams(paramsObject)
		{
            this._IronSource["setVideoServerParams"](paramsObject, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Video Server Parameters");
		}

		_ClearVideoServerParams()
		{
            this._IronSource["clearVideoServerParams"]((e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Clear Video Server Parameters");
		}

		_SetShouldTrackNetworkState(isToTrack)
		{
            this._IronSource["setShouldTrackNetworkState"](isToTrack, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Should Track Network State");
		}

		//** Advanced Settings - Offerwall **//

		_SetOfferwallServerParams(paramsObject)
		{
            this._IronSource["setOfferwallCustomParams"](paramsObject, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Offerwall Custom Server Parameters");
		}

		_SetOfferwallClientSideCallbacks(isToReceive)
		{
            this._IronSource["setClientSideCallbacks"](isToReceive, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Offerwall ClientSide Callbacks");
		}

		//** Rewarded Video **//

		_ShowRewardedVideo(placementName)
		{
            this._IronSource["showRewardedVideo"](placementName, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Show Rewarded Video");
		}

		_GetVideoPlacementInfo(placementName)
		{
			const EVENT_NAME = "get-video-placement-info";

            this._IronSource["getVideoPlacementInfo"](placementName, (...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Get Rewarded Video Placement Info");
		}

		_SetDynamicUserId(dynamicUserId)
		{
            this._IronSource["setDynamicUserID"](dynamicUserId, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Set Dynamic User Id");
		}

		_CheckVideoIfAvailable()
		{
			const EVENT_NAME = "check-video-if-available";

            this._IronSource["isRewardVideoAvailable"]((...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Check Rewarded Video If Available");
		}
		...
		_CreateInterstitial()
		{
            this._IronSource["createInterstitial"]((e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Create Interstitial");
		}

		_ShowInterstitial(placementName)
		{
            this._IronSource["showInterstitial"](placementName, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Show Interstitial");
		}

		_CheckInterstitialIfReady()
		{
			const EVENT_NAME = "check-interstitial-if-ready";

            this._IronSource["isInterstitialReady"]((...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Check Interstitial If Ready");
		}

		//** Banner **//

		_CreateBanner(placementName, isTop, sizeOption, customWidth, customHeight, isAutoShow, isAdaptive)
		{
            this._IronSource["createBanner"](placementName, isTop, sizeOption, customWidth, customHeight, isAutoShow, isAdaptive, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Create Banner");
		}

		_ShowBanner()
		{
			const EVENT_NAME = "show-banner";
			
            this._IronSource["showBanner"]((...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Show Banner");
		}

		_HideBanner()
		{
			const EVENT_NAME = "hide-banner";

            this._IronSource["hideBanner"]((...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Hide Banner");
		}

		_DestroyBanner()
		{
			const EVENT_NAME = "destroy-banner";

            this._IronSource["destroyBanner"]((...e) => { this.FireCallback(EVENT_NAME, e); }, (...err) => { this.FireFailureCallback(EVENT_NAME, err) });
            this._LogAction("Destroy Banner");
		}

		//** Offerwall **//

		_ShowOfferwall(placementName)
		{
            this._IronSource["showOfferwall"](placementName, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Show Offerwall");
		}

		_GetOfferwallCredits()
		{
            this._IronSource["getOfferwallCredits"]((e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction("Get Offerwall Credits");
		}

		//** Interface **//

		_SetMetaData(key, value, log = null)
		{
            this._IronSource["setMetaData"](key, value, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction(log ? log : "Set Metadata");
		}

		_SetMetaDataBool(key, toggle, log = null)
		{
            this._IronSource["setMetaDataBool"](key, toggle, (e) => {}, (...err) => { this.FireSilentFailureCallback(err); });
            this._LogAction(log ? log : "Set Metadata (Boolean)");
		}
	};
